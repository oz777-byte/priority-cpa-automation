...
const RULES_RAW: Omit<AccountingRule, 'id'>[] = [
  {
    ...
    rules: [
      ...
      'vat_reporting_date מוגדר אוטומטית = today (תקופת הדיווח)',
      'בדיקת חוק 6 חודשים: אם invoice.date > 180 יום → JE ייבנה ללא שורת מע"מ',
      'בדיקת dealer_status: אם supplier="exempt" → SUPPLIER_EXEMPT_DEALER במקום',
      'שיעור מע"מ מחושב היסטורית לפי תאריך האספקה (15.5/16/17/18%)',
    ],
    perCompanyOverrides: [
  ...
  {
    code: 'SUPPLIER_EXEMPT_DEALER',
    title: 'ספק עוסק פטור',
    icon: ShieldOff,
    status: 'auto-with-warning',
    category: 'supplier',
    oneLiner: 'ספק שמסומן כעוסק פטור — JE ללא שורת מע"מ, לא נכלל בתשומות 874.',
    description:
      'עוסק פטור (מחזור עד ~120K ₪/שנה) אינו רשאי לגבות מע"מ, ולכן החשבונית שלו אינה חשבונית מס אלא חשבונית עסקה / קבלה. הלקוח (חברה בע"מ במקרה שלנו) לא יכול לקזז מע"מ. המערכת מזהה את הספק לפי הדגל dealer_status="exempt" במאסטר ובונה JE 2 שורות בלבד. החשבונית לא תיכלל בתשומות PCN874 — קריטי כי עוסק פטור לא מדווח גם בצד שלו.',
    triggers: [
      'suppliers.dealer_status = "exempt" במאסטר',
      'בעת יצירת JE: zero out total/subtotal delta → אין שורת מע"מ',
    ],
    jeStructure: 'רשומה אחת, 2 שורות (DR הוצאה / CR ספק). ללא שורת מע"מ תשומות.',
    example: {
      description: 'חשבונית עסקה מיועצת עוסקת פטורה — 800 ₪',
      invoice: { number: 'EX-001', supplier: 'יועצת לעסקים', subtotal: 800, total: 800 },
      je: [
        { account: '504-0', side: 'DR', amount: '800.00', label: 'שירותים מקצועיים' },
        { account: '200xxx', side: 'CR', amount: '800.00', label: 'ספק (יועצת)' },
      ],
      notes: [
        'subtotal = total (אין מע"מ)',
        'אין דרישה למספר הקצאה (חוק ההקצאה לא חל על עוסק פטור)',
        'דווח חשבונית בעוסק פטור = "חשבונית עסקה" או "קבלה", לא "חשבונית מס"',
      ],
    },
    rules: [
      'בלוק על הזנת VAT > 0 — אזהרה אדומה',
      'JE 2 שורות בלבד (אין שורת מע"מ תשומות)',
      'PCN874 לא יכלול את החשבונית בצד התשומות',
      'חוק 6 חודשים לא רלוונטי (ממילא אין מע"מ לקזז)',
    ],
    perCompanyOverrides: [
      'הגדרת dealer_status פר-ספק במאסטר ספקים',
    ],
  },
  ...
  {
    code: 'AR_POST_INVOICE_DISCOUNT',
    title: 'הנחה לאחר חשבונית',
    icon: Percent,
    status: 'auto',
    category: 'customer',
    oneLiner: 'הקטנת חיוב לקוח אחרי שכבר הופקה חשבונית — שונה מזיכוי מלא.',
    description:
      'מצב שבו לקוח קיבל חשבונית מלאה, ואז ניתנת לו הנחה (פשרה / late payment incentive / מחווה). שונה מ-AR_CREDIT_NOTE שמבטל חשבונית במלואה — כאן החשבונית נשארת בתוקף, רק הסכום מופחת. ה-JE מקטין את ההכנסה (DR), את מע"מ העסקאות (DR), ואת יתרת הלקוח (CR). השפעת המע"מ נכנסת אוטומטית ב-PCN874 הקרוב.',
    triggers: ['post_discount_original_invoice מוגדר על חשבונית הזיכוי'],
    jeStructure: '3 שורות (חשבונית עם מע"מ): DR הכנסה + DR מע"מ עסקאות / CR לקוח. 2 שורות אם פטורה.',
    example: {
      description: 'הנחת 200 ₪ + מע"מ ללקוח על חשבונית INV-100',
      invoice: { number: 'DISC-001', supplier: 'לקוח X', subtotal: 200, total: 236 },
      je: [
        { account: '700-0', side: 'DR', amount: '200.00', label: 'הכנסה (הקטנה)' },
        { account: '220-0', side: 'DR', amount: '36.00', label: 'מע"מ עסקאות (הקטנה)' },
        { account: '120-1', side: 'CR', amount: '236.00', label: 'לקוח X (הקטנת יתרה)' },
      ],
      notes: [
        'החשבונית המקורית נשארת בתוקף',
        'נדרש מסמך תיעוד (הסכמה בכתב, מייל מהלקוח)',
      ],
    },
    rules: [
      'JE עם 3 שורות אם יש מע"מ; 2 שורות אם פטור',
      'אזהרה אם לא צוין post_discount_original_invoice',
      'מקטין את מע"מ העסקאות ב-PCN874 הקרוב',
    ],
    perCompanyOverrides: ['חשבון הכנסה (700-0)', 'חשבון מע"מ עסקאות (220-0)'],
  },
...

  {
    ...
    description:
      'בכל חודש (או פעמיים בחודשיים, לפי תקופת הדיווח של החברה) חובה לשדר לרשות המסים את כל פקודות היומן שהשפיעו על מע"מ. המערכת מקבצת את ה-JE לפי vat_reporting_date (לא תאריך החשבונית — תיקון bug של חשבוניות מאוחרות לפי סעיף 38א), מסננת ספקים פטורים אוטומטית, מפרידה עסקאות 0%/פטורות לרשומה L נפרדת, ומפיקה את הקובץ במבנה הרשמי (Windows-1255, רשומות O/S/T/X). ההפקה נועלת אוטומטית את התקופה. תיקון רטרואקטיבי זמין דרך "פתח לתיקון".',
    triggers: [
    ...
    rules: [
      ...
      'סינון ספקי "exempt" — לא נכללים בתשומות',
      'הפרדה אוטומטית של 0%/פטור ב-מכירות → record code L',
      'ההפקה נועלת אוטומטית את התקופה החשבונאית',
      'תיקון רטרואקטיבי: ניתן לפתוח תקופה נעולה דרך כפתור "פתח לתיקון" — דורש סיבה',
      'קובץ תיקון מסומן בשם "CORRECTION-N" + תיוג ב-DB (correction_sequence)',
      'audit log ב-period_reopens table',
    ],
    perCompanyOverrides: [
      'ע.מ של החברה (חובה — מהגדרות חברה, 9 ספרות)',
      'בסיס דיווח (accrual / cash) — vat_basis',
      'תדירות דיווח (חודשי / דו-חודשי / שנתי) — vat_filing_frequency',
    ],
  },
