      ...
      <section className="bg-white border border-ink-200 rounded-xl p-4 space-y-3 print:hidden">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <form
            ...
            action={`/dashboard/c/${company.id}/reports/vat`}
          >
      ...
      <section className="bg-white border border-ink-200 rounded-xl overflow-hidden">
        <div className="flex items-center justify-between px-4 py-2 border-b border-ink-100 bg-ink-50/40">
          ...
          <a
            href={csvUrl}
            download
            className="px-3 py-1.5 text-xs text-accent-600 border border-accent-200 hover:bg-accent-50 rounded-md flex items-center gap-1.5 print:hidden"
          >
            <Download size={12} />
            ייצוא CSV
          </a>
...
function PresetLink({
  ...
  const href = `/dashboard/c/${companyId}/reports/vat?preset=${preset}&from=${range.from}&to=${range.to}`;
  return (
    <Link
      href={href}
